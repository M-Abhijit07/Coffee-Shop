...
<?php
	...
	if(isset($_GET['id'])){
		...
		$relatedProduct = $conn->query("SELECT * FROM products WHERE type='$singleProduct->type' AND id!='$singleProduct->id'");
		$relatedProduct->execute();

		$allrelatedProducts = $relatedProduct->fetchAll(PDO::FETCH_OBJ);
	}
